...
- **Artifact download**: when a response includes `instructions.artifactUrl`,
  ...
  (this skill's own pagination discipline — keyset over OFFSET — is in
...
- Large responses must be paged through in-context using
  `fetch_more(artifactId=..., cursor=...)`; keyset pagination for org
  queries is covered in `references/diff-method.md`.
