...
- **Artifact download**: when a response includes `instructions.artifactUrl`,
  ...
  Page or download per the artifact instructions in the response itself.
...
- Large responses must be paged through in-context using
  `fetch_more(artifactId=..., cursor=...)`, following the artifact
  instructions returned with the response.
