...
# These were previously imported from plugins/salesforce-core-skills/shared/hooks/scripts/
...
class EnhancedFlowValidator:
    ...
    def _validate_design_naming(self) -> dict:
            ...
            advisory.append(
                {
                    ...
                    if naming_results["suggested_names"]
        ...
        if (
            "element_naming_issues" in naming_results
            and len(naming_results["element_naming_issues"]) > 0
        ):
        ...
        if (
            "variable_naming_issues" in naming_results
            and len(naming_results["variable_naming_issues"]) > 0
        ):
    ...
    def _has_dml_in_loops(self) -> bool:
        ...
        for loop in loops:
            ...
            exit_connector = loop.find(
                "sf:noMoreValuesConnector/sf:targetReference", self.namespace
            )
    ...
    def _get_lookups_without_first_record_only(self) -> list[str]:
        ...
        for lookup in self.root.findall(".//sf:recordLookups", self.namespace):
            ...
            is_likely_collection = any(
                ind.lower() in element_name.lower() for ind in collection_indicators
            )
    ...
    def generate_report(self) -> str:
        ...
        report.append("\n" + "═" * 70)
        report.append(
            f"   Flow Validation Report: {results['flow_name']} (API {results['api_version']})"
        )
        report.append("═" * 70)
        ...
        if not results["critical_issues"]:
            ...
            report.append("   ✅  ALWAYS use metadata_create via Salesforce MCP server for deployment")
            report.append("═" * 70)
