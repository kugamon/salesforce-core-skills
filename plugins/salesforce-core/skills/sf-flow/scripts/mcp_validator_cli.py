#!/usr/bin/env python3
"""
Flow MCP Validator CLI
========================

Command-line entry point for validating Flow deployments via Salesforce MCP.

Validates metadata_create, metadata_update, and tooling_api_dml calls
for Flow and FlowDefinition types with 110-point scoring.
...
def format_report(result: dict) -> str:
    """Format Flow deployment validation result as a human-readable report."""
    lines = []
    ...
    lines.append("=" * 60)
    lines.append("  Flow MCP Deployment Validation")
    lines.append("=" * 60)
    lines.append("")
    ...
    if status == "scored":
        score = result.get("score", result.get("overall_score", 0))
        max_score = result.get("max_score", result.get("total_max", 110))
        ...
        lines.append("  SKIPPED -- not a Flow deployment")
    else:
        lines.append(f"  STATUS: {status}")
